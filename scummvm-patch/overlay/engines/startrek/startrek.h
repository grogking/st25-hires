...
private:
	...
	/** Load an .R3S ship view (36-byte matrix header + SHP-style bitmap). */
	Bitmap *loadR3SBitmap(const Common::String &name);
	/** Place / replace the bridge viewscreen target with an .R3S (or clear if empty). */
	void setBridgeTargetFromR3S(const Common::String &r3sBaseName);
	/** Bridge viewscreen: steer (if flight mode), optional Z advance, draw stars + R3s. */
	void updateBridgeStarfield(bool advanceFlight = true);
	/** Turn ship from flight-cursor offset (entcur / mouseControllingShip). */
	void updateBridgeShipSteering();
...
public:
	...
	// UNPROVEN vs EXE — placeholder until entcur path is broken live. See BRIDGE-FIDELITY.md.
	double _bridgeYaw;
	double _bridgePitch;
